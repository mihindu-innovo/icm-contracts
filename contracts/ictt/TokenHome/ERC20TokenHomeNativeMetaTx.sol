...
/**
 * @title ERC20TokenHomeNativeMetaTx
 * @notice ERC20TokenHome with native meta-transaction support for Ava Cloud relayer.
 ...
 */
contract ERC20TokenHomeNativeMetaTx is ERC20TokenHomeUpgradeable, EIP712, Nonces {
    ...
    constructor(
        ...
        EIP712("ERC20TokenHomeNativeMetaTx", "1.0.0")
    {
        initialize(
    ...
    function metaSend(
        ...
        require(deadline == 0 || deadline >= block.timestamp, "ERC20TokenHomeNativeMetaTx: expired");
        ...
        require(signer == from, "ERC20TokenHomeNativeMetaTx: invalid signature");
    ...
    /**
     * @notice Execute a meta-transaction for adding collateral
     * @param from The address of the user who signed the meta-transaction
     * @param remoteBlockchainID The remote blockchain ID
     * @param remoteTokenTransferrerAddress The remote token transferrer address
    ...
    function metaAddCollateral(
        ...
        bytes32 structHash = keccak256(abi.encode(
            keccak256("AddCollateral(address from,bytes32 remoteBlockchainID,address remoteTokenTransferrerAddress,uint256 amount,uint256 nonce,uint256 deadline)"),
            from,
            remoteBlockchainID,
        ...
        // Override _msgSender for this call
        _executeAsSender(from, abi.encodeCall(this.addCollateral, (remoteBlockchainID, remoteTokenTransferrerAddress, amount)));
        
        emit MetaTransactionExecuted(from, nonce, bytes32(bytes4(keccak256("addCollateral(bytes32,address,uint256)"))));
    }
    ...
    function _executeAsSender(address sender, bytes memory data) internal {
        ...
        if (!success) {
            ...
            if (result.length > 0) {
                ...
                revert("ERC20TokenHomeNativeMetaTx: meta-transaction failed");
            }
        }
    }
