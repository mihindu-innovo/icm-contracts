 ...
 */
abstract contract TokenHomeERC2771 is
    ITokenHome,
    ...
    ERC2771Context
{
     ...
     *
     * @custom:storage-location erc7201:avalanche-ictt.storage.TokenHomeERC2771
     */
    struct TokenHomeERC2771Storage {
     ...
     * keccak256(abi.encode(uint256(keccak256("avalanche-ictt.storage.TokenHomeERC2771")) - 1)) & ~bytes32(uint256(0xff));
     */
    bytes32 public constant TOKEN_HOME_ERC2771_STORAGE_LOCATION =
        0x9426912b5a9db88acbe872c934fdd0a46c436c6dcba332d649c4d57c7bc9e600;

    // solhint-disable ordering
    function _getTokenHomeERC2771Storage() private pure returns (TokenHomeERC2771Storage storage $) {
        // solhint-disable-next-line no-inline-assembly
        assembly {
            $.slot := TOKEN_HOME_ERC2771_STORAGE_LOCATION
        }
    }
    ...
    function __TokenHomeERC2771_init(
        ...
        __TokenHomeERC2771_init_unchained(tokenAddress, tokenDecimals);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __TokenHomeERC2771_init_unchained(
        address tokenAddress,
        uint8 tokenDecimals
    ) internal onlyInitializing {
        require(tokenAddress != address(0), "TokenHomeERC2771: zero token address");
        require(
            tokenDecimals <= TokenScalingUtils.MAX_TOKEN_DECIMALS,
            "TokenHomeERC2771: token decimals too high"
        );
        TokenHomeERC2771Storage storage $ = _getTokenHomeERC2771Storage();
        $._blockchainID =
    ...
    function _registerRemote(
        ...
        TokenHomeERC2771Storage storage $ = _getTokenHomeERC2771Storage();
        require(remoteBlockchainID != bytes32(0), "TokenHomeERC2771: zero remote blockchain ID");
        require(
            remoteBlockchainID != $._blockchainID, "TokenHomeERC2771: cannot register remote on same chain"
        );
        require(
            remoteTokenTransferrerAddress != address(0),
            "TokenHomeERC2771: zero remote token transferrer address"
        );
        require(
            !$._registeredRemotes[remoteBlockchainID][remoteTokenTransferrerAddress].registered,
            "TokenHomeERC2771: remote already registered"
        );
        require(
            message.remoteTokenDecimals <= TokenScalingUtils.MAX_TOKEN_DECIMALS,
            "TokenHomeERC2771: remote token decimals too high"
        );
        require(
            message.homeTokenDecimals == $._tokenDecimals, "TokenHomeERC2771: invalid home token decimals"
        );
    ...
    function _send(SendTokensInput memory input, uint256 amount) internal sendNonReentrant {
        ...
        require(input.multiHopFallback == address(0), "TokenHomeERC2771: non-zero multi-hop fallback");
    ...
    function _addCollateral(
        ...
        RemoteTokenTransferrerSettings memory remoteSettings =
            $._registeredRemotes[remoteBlockchainID][remoteTokenTransferrerAddress];
        require(remoteSettings.registered, "TokenHomeERC2771: remote not registered");
        require(remoteSettings.collateralNeeded > 0, "TokenHomeERC2771: zero collateral needed");
    ...
    function _receiveTeleporterMessage(
        ...
        if (transferrerMessage.messageType == TransferrerMessageType.SINGLE_HOP_SEND) {
            ...
            require(
                ...
                "TokenHomeERC2771: mismatched source blockchain ID"
            );
            require(
                payload.originTokenTransferrerAddress == originSenderAddress,
                "TokenHomeERC2771: mismatched origin sender address"
            );
    ...
    function _processReceivedTransfer(
        ...
        require(remoteSettings.registered, "TokenHomeERC2771: remote not registered");
        require(remoteSettings.collateralNeeded == 0, "TokenHomeERC2771: remote not collateralized");
        ...
        require(homeAmount > 0, "TokenHomeERC2771: zero token amount");
        return homeAmount;
    }
    ...
    function _prepareMultiHopRouting(
        ...
        require(amount > fee, "TokenHomeERC2771: insufficient amount to cover fees");
        amount -= fee;
    ...
    function _prepareSend(
        ...
        RemoteTokenTransferrerSettings memory remoteSettings =
            $._registeredRemotes[remoteBlockchainID][remoteTokenTransferrerAddress];
        require(remoteSettings.registered, "TokenHomeERC2771: remote not registered");
        require(remoteSettings.collateralNeeded == 0, "TokenHomeERC2771: collateral needed for remote");
        ...
        require(scaledAmount > 0, "TokenHomeERC2771: zero scaled amount");
    ...
    function _deductSenderBalance(
        ...
        require(senderBalance >= amount, "TokenHomeERC2771: insufficient token transfer balance");
        $._transferredBalances[remoteBlockchainID][remoteTokenTransferrerAddress] =
    ...
    function _validateSendAndCallInput(
        ...
        require(input.recipientContract != address(0), "TokenHomeERC2771: zero recipient contract address");
        require(input.requiredGasLimit > 0, "TokenHomeERC2771: zero required gas limit");
        require(input.recipientGasLimit > 0, "TokenHomeERC2771: zero recipient gas limit");
        require(
            input.recipientGasLimit < input.requiredGasLimit,
            "TokenHomeERC2771: invalid recipient gas limit"
        );
        require(input.fallbackRecipient != address(0), "TokenHomeERC2771: zero fallback recipient address");
        require(input.secondaryFee == 0, "TokenHomeERC2771: non-zero secondary fee");
    }

    function _validateSendTokensInput(
        SendTokensInput memory input
    ) private pure {
        require(input.recipient != address(0), "TokenHomeERC2771: zero recipient address");
        require(input.requiredGasLimit > 0, "TokenHomeERC2771: zero required gas limit");
        require(input.secondaryFee == 0, "TokenHomeERC2771: non-zero secondary fee");
    }
}