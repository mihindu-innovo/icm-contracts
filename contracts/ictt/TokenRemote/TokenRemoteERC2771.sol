 ...
 */
abstract contract TokenRemoteERC2771 is
    ITokenRemote,
    ...
    ERC2771Context
{
     ...
     *
     * @custom:storage-location erc7201:avalanche-ictt.storage.TokenRemoteERC2771
     */
    struct TokenRemoteERC2771Storage {
     ...
     * keccak256(abi.encode(uint256(keccak256("avalanche-ictt.storage.TokenRemoteERC2771")) - 1)) & ~bytes32(uint256(0xff));
     */
    bytes32 public constant TOKEN_REMOTE_ERC2771_STORAGE_LOCATION =
        0x710d6a9b283d1eda563de594ce4843869b6f128a4baa222422ed94a60b0cef00;

    // solhint-disable ordering
    function _getTokenRemoteERC2771Storage() private pure returns (TokenRemoteERC2771Storage storage $) {
        // solhint-disable-next-line no-inline-assembly
        assembly {
            $.slot := TOKEN_REMOTE_ERC2771_STORAGE_LOCATION
        }
    }
    ...
    function __TokenRemoteERC2771_init(
        ...
        __TokenRemoteERC2771_init_unchained(settings, initialReserveImbalance_, tokenDecimals);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __TokenRemoteERC2771_init_unchained(
        TokenRemoteSettings memory settings,
        uint256 initialReserveImbalance_,
        uint8 tokenDecimals
    ) internal onlyInitializing {
        TokenRemoteERC2771Storage storage $ = _getTokenRemoteERC2771Storage();
        $._blockchainID =
            IWarpMessenger(0x0200000000000000000000000000000000000005).getBlockchainID();
        require(
            settings.tokenHomeBlockchainID != bytes32(0),
            "TokenRemoteERC2771: zero token home blockchain ID"
        );
        require(
            settings.tokenHomeBlockchainID != $._blockchainID,
            "TokenRemoteERC2771: cannot deploy to same blockchain as token home"
        );
        require(settings.tokenHomeAddress != address(0), "TokenRemoteERC2771: zero token home address");
        require(
            settings.tokenHomeDecimals <= TokenScalingUtils.MAX_TOKEN_DECIMALS,
            "TokenRemoteERC2771: token home decimals too high"
        );
        require(
            tokenDecimals <= TokenScalingUtils.MAX_TOKEN_DECIMALS,
            "TokenRemoteERC2771: token decimals too high"
        );
    ...
    function registerWithHome(
        ...
        TokenRemoteERC2771Storage storage $ = _getTokenRemoteERC2771Storage();
        require(!$._isRegistered, "TokenRemoteERC2771: already registered");
    ...
    function _receiveTeleporterMessage(
        ...
        require(
            ...
            "TokenRemoteERC2771: invalid source blockchain ID"
        );
        require(
            originSenderAddress == $._tokenHomeAddress, "TokenRemoteERC2771: invalid origin sender address"
        );
        ...
        if (transferrerMessage.messageType == TransferrerMessageType.SINGLE_HOP_SEND) {
            ...
            revert("TokenRemoteERC2771: invalid message type");
        }
    }
    ...
    function _prepareSend(
        ...
        require(
            ...
            "TokenRemoteERC2771: insufficient tokens to transfer"
        );
    ...
    function _validateSingleHopInput(
        ...
        require(
            ...
            "TokenRemoteERC2771: invalid destination token transferrer address"
        );
        require(secondaryFee == 0, "TokenRemoteERC2771: non-zero secondary fee");
        require(multiHopFallback == address(0), "TokenRemoteERC2771: non-zero multi-hop fallback");
    }
    ...
    function _validateMultiHopInput(
        ...
        require(multiHopFallback != address(0), "TokenRemoteERC2771: zero multi-hop fallback");
    }

    function _validateSendTokensInput(
        SendTokensInput calldata input
    ) private pure {
        require(input.recipient != address(0), "TokenRemoteERC2771: zero recipient address");
        require(input.requiredGasLimit > 0, "TokenRemoteERC2771: zero required gas limit");
        require(
            input.destinationBlockchainID != bytes32(0),
            "TokenRemoteERC2771: zero destination blockchain ID"
        );
        require(
            input.destinationTokenTransferrerAddress != address(0),
            "TokenRemoteERC2771: zero destination token transferrer address"
        );
    }
    ...
    function _validateSendAndCallInput(
        ...
        require(
            input.recipientContract != address(0), "TokenRemoteERC2771: zero recipient contract address"
        );
        require(input.requiredGasLimit > 0, "TokenRemoteERC2771: zero required gas limit");
        require(input.recipientGasLimit > 0, "TokenRemoteERC2771: zero recipient gas limit");
        require(
            input.recipientGasLimit < input.requiredGasLimit,
            "TokenRemoteERC2771: invalid recipient gas limit"
        );
        require(
            input.fallbackRecipient != address(0), "TokenRemoteERC2771: zero fallback recipient address"
        );
    }
}